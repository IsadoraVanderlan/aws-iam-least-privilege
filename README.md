...
1. 1. **Subindo o laboratório:** Enviei o arquivo `template.yaml` para o serviço CloudFormation da AWS, que criou todos os usuários e regras automaticamente em poucos minutos.
...
1. **Verificando os Usuários:** Após o término da automação, a AWS criou com sucesso todos os perfis de acesso direto no painel do IAM conforme planejado.
   ![Usuários Criados no IAM](user.png)

1. **Testando o bloqueio do Operador S3:** Entrei com o usuário operador e tentei listar outros arquivos da conta. A AWS barrou e deu o erro de **Acesso Negado (AccessDenied)**, provando que a trava que criei funcionou. Ele só conseguiu ver o bucket do laboratório.
   ![Validação no CloudShell](permissoes.png)

1. **Limpando tudo:** Após os testes, usei o CloudFormation para deletar todos os recursos criados de forma limpa e segura, evitando qualquer custo na minha conta.
   <br>
